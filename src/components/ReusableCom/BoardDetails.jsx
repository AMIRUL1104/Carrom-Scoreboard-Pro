...
function BoardDetails({ details }) {
  return (
    //  className="w-full overflow-auto bg-[#0d1117] border border-[#1e2836] rounded-2xl"
    <div
      className="
      w-full
    ...
    >
      <table
        //   className="w-full min-w-100 text-sm text-[#e8f0f8]"
        className="w-full min-w-125 max-sm:min-w-83 text-[#e8f0f8] text-xs sm:text-sm lg:text-base"
      >
        ...
        <tbody className="divide-y divide-[#1e2836]">
          {details.map((board) => (
            <tr key={board.id} className="hover:bg-[#060810] transition">
              ...
              <td className="px-0 sm:px-6 py-3 text-center">{board.value}</td>
